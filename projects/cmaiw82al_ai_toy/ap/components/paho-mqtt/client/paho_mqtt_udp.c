...
static int sendPacket(MQTT_CLIENT_T *c, int length)
{
    ...
    if (c->tls_session)
    {
        while(total < length)
        {
            ...
            debug_printf("%s %d fd:%d %d rc:%d length:%d\r\n",__func__,__LINE__,c->tls_session->server_fd.fd,c->sock,rc,length);
            if(rc <= 0)
            {
    ...
    debug_printf("%s %d length:%d,rc:%d\r\n",__func__,__LINE__,length,rc);
...
static int net_read(MQTT_CLIENT_T *c, unsigned char *buf,  int len, int timeout)
{
    int bytes = 0;
    int rc;
    uint32_t start = (uint32_t)bk_get_tick();
    uint32_t end_tick = timeout/bk_get_ms_per_tick();
    ...
    while (bytes < len)
    {
        ...
        if (c->tls_session)
        {
            ...
            if((uint32_t)bk_get_tick() - start > end_tick)
            {
                break;
            }
            else
...
_mqtt_start:
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; i++)
    {
        ...
        debug_printf("topic:%s qos:%d\r\n",c->messageHandlers[i].topicFilter,c->messageHandlers[i].qos);
        ...
        rc = mqtt_subscribe(c, topic, c->messageHandlers[i].qos);
    ...
    while (1)
    {
        ...
        tick_now = bk_get_tick();
        if (((tick_now - c->tick_ping) / bk_get_ticks_per_second()) > (c->keepAliveInterval))
        {
            timeout.tv_sec = 0;
        }
        else
        {
            timeout.tv_sec = c->keepAliveInterval - (tick_now - c->tick_ping) / bk_get_ticks_per_second();
...
_mqtt_restart:
    ...
    net_disconnect(c);
    rtos_delay_milliseconds(bk_get_ticks_per_second() * 5);
    if(c->is_quit)
    {
        bk_printf("mqtt quit\r\n");
        goto _mqtt_exit;
