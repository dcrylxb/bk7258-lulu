...
int webnet_request_parse_method(struct webnet_request *request, char* buffer, int length)
{
    ...
    BK_ASSERT(request->session != NULL);
        ...
        if (str_begin_with(request_buffer, "\r\n"))
        {
            ...
            if (request->content_length && request->method == WEBNET_POST)
            {
                    ...
                    request->content_length < POST_BUFSZ_MAX)
                {
                    ...
                    os_memset(request->query, 0, request->content_length + 1);
    ...
    for (;;)
    {
        if (str_begin_with(request_buffer, "\r\n"))
        {
            ...
            if (request->method == WEBNET_POST && request->content_length > 0) /* POST method */
            {
                ...
                {
                    BK_LOGE(TAG,"content length too long: %d, discard it.", request->content_length);
                ...
                if (read_ptr == NULL)
                {
                    BK_LOGE(TAG,"No memory for request read buffer!");
                    request->query = NULL;
                    return ;
                }
...
struct webnet_request* webnet_request_create()
{
    ...
    if (request != NULL)
    {
        os_memset(request, 0, sizeof(struct webnet_request));
        request->field_copied = 0;
    }
