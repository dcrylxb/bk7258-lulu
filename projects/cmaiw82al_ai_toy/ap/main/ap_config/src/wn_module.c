...
static void _webnet_dofile_handle(struct webnet_session *session, int event)
{
    ...
    {
        size_t readbytes;
        size_t length = ALIGN(WEBNET_SESSION_BUFSZ, 4);
...
/*
    ...
    while (default_files[index] != NULL)
    {
        struct stat file_stat;
         ...
         made a full path 
        os_snprintf(full_path, WEBNET_PATH_MAX, "%s/%s%s",
...
_end_default_files:
    ...
    {
        /* use old full path */
        os_snprintf(full_path, WEBNET_PATH_MAX, "%s/%s", webnet_get_root(), request->path);
        /* normalize path */
        str_normalize_path(full_path);
    }
