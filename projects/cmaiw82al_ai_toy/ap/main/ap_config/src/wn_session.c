...
int webnet_session_get_physical_path(struct webnet_session *session, const char* virtual_path, char* full_path)
{
    ...
    /* made a full path */
    os_snprintf(full_path, WEBNET_PATH_MAX, "%s/%s", webnet_get_root(), virtual_path);
    /* normalize path */
        ...
        const char * reason_phrase)
{
    ...
    os_snprintf(status_line, sizeof(status_line) - 1, "HTTP/1.1 %d ", code);
...
void webnet_session_set_header(struct webnet_session *session, const char* mimetype, int code, const char* title, int length)
{
    ...
    offset = os_snprintf(ptr, end_buffer - ptr, fmt, code, title, WEBNET_SERVER);
    ptr += offset;

    if (code == 401)
    {
        offset = os_snprintf(ptr, end_buffer - ptr, auth, session->request->host);
        ptr += offset;
    }
    if (length >= 0)
    {
        offset = os_snprintf(ptr, end_buffer - ptr, content, mimetype, length,
                             session->request->connection == WEBNET_CONN_CLOSE? "close" : "Keep-Alive");
        ptr += offset;
    }
    else
    {
        offset = os_snprintf(ptr, end_buffer - ptr, content_nolength, mimetype, "close");
        ptr += offset;
    }
...
static void _webnet_session_handle_read(struct webnet_session* session)
{
    ...
    read_length = (int)webnet_session_read(session, (char*)buffer_ptr, session->buffer_length - session->buffer_offset);
